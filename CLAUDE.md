...
| `PARACHUTE_AGENT_ALERT_CHANNEL` | (unset) | **Daemon:** name of an already-configured telegram channel (channels.json) whose bot sends scheduled-job failure/recovery alerts (agent's risk register R1). Must be set TOGETHER with `PARACHUTE_AGENT_ALERT_CHAT_ID` — either alone is a partial config (logged, alerts disabled). Both unset → alerting is off (silent, no misconfig warning). |
| `PARACHUTE_AGENT_ALERT_CHAT_ID` | (unset) | **Daemon:** the Telegram chat id (DM or group) the job alert is sent to, via the `PARACHUTE_AGENT_ALERT_CHANNEL` bot. See above. |
